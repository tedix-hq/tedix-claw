...
import { ensureGateway, findExistingGateway, syncToR2 } from "./gateway";
import { adminUi, api, cdp, debug, publicRoutes } from "./routes";
...
async function scheduled(
  ...
  const gatewayProcess = await findExistingGateway(sandbox);
  if (!gatewayProcess) {
